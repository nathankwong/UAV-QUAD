...
psi_cmd_nom = Signals(dc_offset=MAV.psi0,
                        amplitude=0.0,
                        start_time=1000.0,
                        frequency=0.01)

north_cmd_nom = Signals(dc_offset=-MAV.north0,
                        amplitude=0.0,
                        start_time=1000.0,
                        frequency=0.015)

east_cmd_nom = Signals(dc_offset=MAV.east0,
                        amplitude=0.,
                        start_time=1000.0,
                        frequency=0.015)

down_cmd_nom = Signals(dc_offset=MAV.down0,
                        amplitude=0.,
                        start_time=1000.0,
                        frequency=0.015)

def run_sim(sim: MsgSimParams, init_state: Optional[DynamicState] = None, \
        mav_view: Optional[MavViewer] = None, data_view: Optional[DataViewer] = None, \
        use_wind: bool = False, gust_params: Optional[MsgGustParams] = None, \
        psi_cmd:  Signals = psi_cmd_nom, \
        north_cmd: Signals = north_cmd_nom, \
        east_cmd: Signals = east_cmd_nom,\
        down_cmd: Signals = down_cmd_nom
        ) \
    ...
    def current_wind() -> types.WindVector:
        zero_wind_vec = np.zeros([6,1])
        return cast(types.WindVector, zero_wind_vec)
    if use_wind:
    ...
    while sim_time < sim.end_time:
        # -------autopilot commands-------------
        commands.psi_command = psi_cmd.square(sim_time)
        commands.north_command = north_cmd.square(sim_time)
        commands.east_command = east_cmd.square(sim_time)
        commands.down_command = down_cmd.square(sim_time)
