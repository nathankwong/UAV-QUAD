...
class Waypoint:
    ...
    def __str__(self) -> str:
        ...
        """
        msg: str = ""
        msg += "\nned = (" + str(self.ned.item(0)) + ", " + str(self.ned.item(1)) + ", " + str(self.ned.item(2)) + ")"
...
class MsgWaypoints:
    ...
    def prepend(self, ned: npt.NDArray[Any] =np.array([[0, 0, 0]]).T, airspeed: float=0,
            course: float =np.inf, cost: float=0, parent: int =0, connect_to_goal: int =0) -> bool:
    ...
    def get_ned(self, index: int) -> npt.NDArray[Any]:
        ...
        return self.ned[:, index:index+1] # Ensures the output is a column vector
