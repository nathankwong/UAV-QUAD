...
class WindSimulation:
    ...
    def __init__(self, Ts: float, gust_params: Optional[MsgGustParams] = None, gust_flag: bool = True) -> None:
        ...
        #Array=[a1 a2 a3] coeffs!
        
        num_1 = sigma_u*np.sqrt(2.*Va/(np.pi*Lu))
        dem_1 = Va/Lu
        self.u_w = TransferFunction(num=np.array([[num_1]]),
                                     den=np.array([[1,dem_1]]),
                                     Ts=Ts)

        num_2_1 = sigma_v*np.sqrt(3.*Va/(np.pi*Lv))
        num_2_2 = sigma_v*np.sqrt(3.*Va/(np.pi*Lv))*(Va/(np.sqrt(3.)*Lv))

        dem_2_1 = 1
        dem_2_2 = 2*(Va/Lv)
        dem_2_3 = (Va/Lv)**2

        self.v_w = TransferFunction(num=np.array([[num_2_1, num_2_2]]),
                                     den=np.array([[dem_2_1, dem_2_2, dem_2_3]]),
                                     Ts=Ts)

        num_3_1 = sigma_w*np.sqrt(3.*Va/(np.pi*Lw))
        num_3_2 = sigma_w*np.sqrt(3.*Va/(np.pi*Lw))*(Va/(np.sqrt(3.)*Lw))

        dem_3_1 = 1
        dem_3_2 = 2*Va/Lw
        dem_3_3 = (Va/Lw)**2

        self.w_w = TransferFunction(num=np.array([[num_3_1, num_3_2]]),
                                     den=np.array([[dem_3_1, dem_3_2, dem_3_3]]),
                                     Ts=Ts)
        self._Ts = Ts
