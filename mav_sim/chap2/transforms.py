...
def trans_i_to_v(pose: Pose, p_i: Points) -> Points:
    ...
    """
    v_north=p_i[0]-pose.north
    v_east=p_i[1]-pose.east
    v_down=np.abs(-pose.altitude-p_i[2])
    p_i=np.array([[v_north,v_east,v_down]])
...
def trans_v_to_i(pose: Pose, p_v: Points) -> Points:
    ...
    Returns:
        p_i: Point represented in the inertial frame
    """
    i_north=p_v[0]+pose.north
    i_east=p_v[1]+pose.east
    i_down=p_v[2]-pose.altitude
    p_v=np.array([[i_north,i_east,i_down]])
...
def trans_i_to_b(pose: Pose, p_i: Points) -> Points:
    ...
    p_i=rot_v_to_b(pose.psi,pose.theta,pose.phi)@trans_i_to_v(pose,p_i)
    return cast(Points,p_i)
...
def trans_b_to_i(pose: Pose, p_b: Points) -> Points:
    ...
    p_v=cast(Points, np.transpose(rot_v_to_b(pose.psi,pose.theta,pose.phi))@ p_b)
    p_b=trans_v_to_i(pose,p_v)
    return p_b
