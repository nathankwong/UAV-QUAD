...
from mav_sim.tools.types import Quaternion  # Type for a numpy matrix
...
def compute_model(trim_state: types.DynamicState, trim_input: MsgDelta) -> None:
    ...
    Va_trim, alpha_trim, theta_trim= compute_tf_model(trim_state)
    ...
    with open('model_coef.py', 'w') as file:
        ...
        file.write('u_trim = np.array([[%f, %f, %f, %f]]).T\n' %
                (trim_input.w1, trim_input.w2, trim_input.w3, trim_input.w4))
...
def compute_tf_model(trim_state: types.DynamicState) \
    ...
    quat = cast(Quaternion, trim_state[IND.QUAT])
    _, theta_trim, _ = Quaternion2Euler(quat)
    ...
    return Va_trim, alpha_trim, theta_trim
    # , a_phi1, a_phi2, a_theta1, a_theta2, a_theta3, a_V1, a_V2, a_V3
...
def f_euler(x_euler: types.DynamicStateEuler, delta: MsgDelta) -> types.DynamicStateEuler:
    ...
    f = derivatives(x_quat, forces_moments(x_quat, delta, Va))
...
def df_du(x_euler: types.DynamicStateEuler, delta: MsgDelta) -> npt.NDArray[Any]:
    ...
    f = f_euler(x_euler, delta)
    delta_eps = MsgDelta(w1=delta.w1,
                         w2=delta.w2,
                         w3=delta.w3,
                         w4=delta.w4)
    for i in range(0, 4):
