...
from mav_sim.state_plotter.plotter import Plotter
from mav_sim.state_plotter.plotter_args import PlotboxArgs


class DataViewer:
    """Plotting capabilities for viewing states, inputs, noise, etc
    """
    def __init__(self) -> None:
        ...
        # define second row
        v_a_plots = PlotboxArgs(plots=['Va', 'Va_e', 'Va_c'],
        ...
        beta_plots = PlotboxArgs(plots=['beta', 'beta_e'],
                                 ...
                                 rad2deg=True,
                                 time_window=time_window_length)
        v_g_plots = PlotboxArgs(plots=['Vg', 'Vg_e'],
                               labels={'left': 'Vg(m/s)', 'bottom': 'Time (s)'},
                               time_window=time_window_length)
        second_row = [v_a_plots, alpha_plots, beta_plots, v_g_plots]
    ...
    def update(self, true_state: MsgState, estimated_state: MsgState, \
        commanded_state: MsgState, delta: MsgDelta, ts: float) -> None:
        """Update each of the plots

        Args:
            true_state: The actual state
            estimated_state: The current estimate of the state
            commanded_state: The state being commanded
            delta: Vehicle control commands
            ts: Time step
        """
        ...
        delta_list = [delta.T, delta.tx, delta.ty, delta.tz]
    ...
    def reset(self, start_time: float) -> None:
        """reset all data to begin plotting again starting at time = start_time
        """
        self.plotter.reset_data(start_time)
